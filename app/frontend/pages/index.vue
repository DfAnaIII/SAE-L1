  ...
  <NuxtLayout>
    <div class="flex flex-col items-center h-screen justify-center">
      ...
      <div class="flex flex-row items-center justify-between h-full w-full tracking-wide">
        ...
        <Separator orientation="vertical" class="my-2 h-1/2" />
        ...
        <div class="flex flex-col items-center justify-center h-full tracking-wide w-full">
          <Card class="w-3/4 h-4/5">
            ...
            <CardContent class="h-3/4">
              <div v-if="!outputContent" class="flex flex-col space-y-3">
            ...
            <CardFooter class="flex flex-row items-center justify-center gap-2 w-full">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger as-child>
                    <Button class="w-full" @click="refreshOutput">
                      Rafraîchir
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent side="bottom">
...
async function sendContentTest() {
  try {
    const response = await fetch(`${config.public.apiUrl}/test`, {
      method: 'POST',
      headers: {
...
async function sendContent() {
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      outputContent.value = data.result || 'Traitement effectué';
...
function refreshOutput() {
  // Vous pouvez implémenter une requête pour obtenir les derniers résultats
  // ou simplement effacer le résultat actuel
  outputContent.value = '';
}
